...
use openorb::discovery::{NetworkDiscovery, PortScanner, ScanMethod, SynScanner, ParsedVersion};
use openorb::vulndb::{
...
#[command(author, version, about = "OpenOrb - Network Vulnerability Scanner", long_about = None)]
struct Cli {
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Server { host, port, db } => {
            openorb::api::run_server(&db, &host, port).await?;
        }
...
fn run_status(scan_id: &str, db_path: &str, detail: bool) -> Result<()> {
    ...
    match status.step {
        ...
        2 => println!("\n\x1b[1mNext:\x1b[0m openorb match {}", scan_id),
        _ => {}
    }
